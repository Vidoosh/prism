...
| `INTERNAL_API_URL` | No | Backend URL for server-side requests (defaults to `NEXT_PUBLIC_API_URL`; set to `http://backend:8000` in Docker Compose). **On Vercel**, use the same public HTTPS URL as `NEXT_PUBLIC_API_URL` unless you use a split internal route. |
...
See root **[DEPLOYMENT.md](../DEPLOYMENT.md)** — use **Root Directory** `prism/frontend`, set `NEXT_PUBLIC_API_URL`, `INTERNAL_API_URL`, and Sanity vars in the project dashboard.

### Docker / Compose

Production images use **`output: "standalone"`** only when **`VERCEL` is unset** during `next build`, matching [`frontend/Dockerfile`](Dockerfile). See [`docker-compose.yml`](../docker-compose.yml).
